# 事件屏蔽器油猴插件

这是一个油猴插件，用于屏蔽浏览器中的各种事件，包括鼠标、键盘、触摸等事件。

## 功能特性

- 可配置屏蔽的事件类型
- 提供美观的现代化配置界面
- 支持中英文双语切换
- 流畅的动画效果和交互体验
- 支持多种事件类型的屏蔽
- **增强模式**：通过重写 addEventListener 提供更强大的事件阻止能力

## 界面特色

- 渐变紫色主题设计
- 卡片式布局，清晰直观
- 自定义复选框样式
- 悬停动画效果
- 毛玻璃背景遮罩
- 响应式设计
- 自定义滚动条样式
- 增强模式开关控制

## 支持屏蔽的事件类型

- 鼠标事件（click, dblclick, mousedown, mouseup等）
- 键盘事件（keydown, keyup, keypress）
- 触摸事件（touchstart, touchmove, touchend等）
- 滚轮事件（wheel, mousewheel等）
- 焦点事件（focus, blur等）
- 表单事件（submit, change, input等）
- 剪贴板事件（copy, cut, paste）
- 拖拽事件（drag, dragstart, dragend等）

## 安装方法

1. 确保浏览器已安装Tampermonkey扩展
2. 打开`event-blocker.user.js`文件
3. 复制文件内容
4. 在Tampermonkey中点击"添加新脚本"
5. 粘贴复制的内容并保存
6. 刷新浏览器页面以启用插件

## 使用方法

1. 点击Tampermonkey图标
2. 选择"配置事件屏蔽"（中文）/ "Configure Event Blocker"（英文）
3. 在弹出的配置窗口中选择要屏蔽的事件类型
4. **增强模式**：在配置窗口左上角可以开启/关闭增强模式，提供更强大的事件阻止能力
5. 点击右上角的语言切换按钮（🇨🇳 中文 / 🇺🇸 EN）可切换界面语言
6. 点击"保存配置"按钮应用配置
7. 页面会自动刷新以应用新的配置

### 增强模式说明

- **传统模式**：通过在捕获阶段添加监听器来阻止事件
- **增强模式**：通过重写 addEventListener 方法，在事件注册时就进行拦截
- 增强模式提供更全面的事件阻止能力，适合需要严格控制事件的场景
- 无论是否启用增强模式，配置窗口的事件始终正常工作

### 语言设置

- 配置窗口右上角有语言切换按钮
- 点击可在中文和英文之间切换
- 语言选择会自动保存，下次打开时保持不变

## 注意事项

- 屏蔽某些事件可能会影响网站的正常功能
- 请根据实际需要选择性地屏蔽事件
- 如有问题，请尝试禁用插件或调整配置
- 增强模式会重写浏览器的原生方法，虽然经过安全处理，但仍建议谨慎使用

## 更新历史

请查看 [CHANGELOG.md](CHANGELOG.md) 文件了解详细的更新历史。